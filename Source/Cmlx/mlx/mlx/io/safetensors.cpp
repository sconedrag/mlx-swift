...
// W4 (mmap weight serving): load via a memory-mapped reader so the Load
// primitive's zero-copy path (Load::eval_cpu) can build Metal buffers over the
// file-backed pages instead of malloc + read.
SafetensorsLoad load_safetensors_mmap(const std::string& file, StreamOrDevice s) {
  return load_safetensors(std::make_shared<io::MmapFileReader>(file), s);
}
