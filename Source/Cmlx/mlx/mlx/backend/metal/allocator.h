...
class MetalAllocator : public allocator::Allocator {
 ...
 public:
  ...
  /// W4 (mmap weight serving): create a Buffer wrapping externally-owned mmap'd
  /// memory via Metal's newBufferWithBytesNoCopy. NOT added to the buffer cache,
  /// residency set, or active-memory accounting — the caller (MmapFileReader)
  /// owns the memory lifetime.
  Buffer malloc_nocopy(void* ptr, size_t size);
 ...
 private:
  ...
  // W4: track mmap-backed buffers so free()/cache/residency skip external memory
  std::unordered_set<MTL::Buffer*> nocopy_buffers_;
};
