...
// W4 (mmap weight serving): mmap'd buffers use Tracked hazard mode so the GPU
// cannot read a page mid-eviction — prevents silent corruption when iOS reclaims
// clean (file-backed) weight pages during compute. (Study 12 RQ4.)
constexpr MTL::ResourceOptions mmap_resource_options =
...
Buffer MetalAllocator::malloc_nocopy(void* ptr, size_t size) {
  if (!ptr || size == 0) {
    return Buffer{nullptr};
  }
  ...
  // Metal-cpp newBuffer 4-parameter overload = newBufferWithBytesNoCopy.
  // Passing nullptr deallocator = caller (MmapFileReader) manages memory lifetime.
  MTL::Buffer* buf = device_->newBuffer(ptr, size, mmap_resource_options, nullptr);

  if (!buf) {
    std::ostringstream msg;
    msg << "[metal::malloc_nocopy] newBufferWithBytesNoCopy failed for " << size
        << " bytes. Pointer may not be page-aligned (requires " << vm_page_size
        << " byte alignment).";
    throw std::runtime_error(msg.str());
  }

  // Track as nocopy so free() releases only the wrapper, not the mmap'd memory.
  std::unique_lock lk(mutex_);
  nocopy_buffers_.insert(buf);
  // Do NOT add to buffer_cache_ (we don't own this memory)
  // Do NOT add to residency_set_ (OS manages paging for mmap'd pages)
  // Do NOT increment active_memory_ (this is external memory)
...
void MetalAllocator::free(Buffer buffer) {
  ...
  // W4: mmap-backed (nocopy) buffer — release only the Metal wrapper; the
  // underlying mmap'd memory is owned by MmapFileReader. Skip cache / residency
  // / active-memory accounting (we never counted it on the way in).
  auto nocopy_it = nocopy_buffers_.find(buf);
  if (nocopy_it != nocopy_buffers_.end()) {
    nocopy_buffers_.erase(nocopy_it);
    buf->release();
    return;
  }
