...
// Resource options for mmap'd buffers: use Tracked hazard mode to prevent
// silent data corruption if OS evicts pages during GPU compute.
...
Buffer MetalAllocator::malloc_nocopy(void* ptr, size_t size) {
  ...
  // Metal-cpp newBuffer 4-parameter overload = NoCopy variant (MTLDevice.hpp:935-937)
  // Passing nullptr deallocator = caller (MmapFileReader) manages memory lifetime.
  MTL::Buffer* buf = device_->newBuffer(
      ptr, size, mmap_resource_options, nullptr);

  if (!buf) {
    std::ostringstream msg;
    msg << "[metal::malloc_nocopy] newBufferWithBytesNoCopy failed for "
        << size << " bytes. Pointer may not be page-aligned (requires "
        << vm_page_size << " byte alignment).";
    throw std::runtime_error(msg.str());
  }

  // Track as nocopy so free() knows not to cache or release
  std::unique_lock lk(mutex_);
...
void MetalAllocator::free(Buffer buffer) {
  ...
  // Check if this is a nocopy (mmap-backed) buffer
  auto it = nocopy_buffers_.find(buf);
  if (it != nocopy_buffers_.end()) {
    // Nocopy buffer: do NOT recycle, do NOT decrement active_memory_,
    // do NOT add to cache. Just release the Metal buffer object
    // (this does NOT free the underlying mmap'd memory).
    nocopy_buffers_.erase(it);
    buf->release();
    return;
  }

  // Standard path (existing implementation):
  active_memory_ -= buf->length();
  if (get_cache_memory() < max_pool_size_) {
