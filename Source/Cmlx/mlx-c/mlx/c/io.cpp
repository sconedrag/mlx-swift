...
// W4 (mmap weight serving): C binding for the memory-mapped safetensors loader.
extern "C" int mlx_load_safetensors_mmap(
    mlx_map_string_to_array* res_0,
    mlx_map_string_to_string* res_1,
    const char* path,
    const mlx_stream s) {
  try {
    {
      auto [tpl_0, tpl_1] =
          mlx::core::load_safetensors_mmap(std::string(path), mlx_stream_get_(s));
      mlx_map_string_to_array_set_(*res_0, tpl_0);
      mlx_map_string_to_string_set_(*res_1, tpl_1);
    };
  } catch (std::exception& e) {
